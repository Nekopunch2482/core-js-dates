...
function getCountWeekendsInMonth(month, year) {
  return Array.from({ length: getCountDaysInMonth(month, year) }, (_, i) =>
...
function getWeekNumberByDate(date) {
  ...
  dateNow.setUTCDate(dateNow.getUTCDate() + 5 - (dateNow.getUTCDay() || 7));

  const yearStart = new Date(Date.UTC(dateNow.getUTCFullYear(), 0, 1));

  const weekMs = 7 * 24 * 60 * 60 * 1000;

  const diffMs = dateNow - yearStart;

  const weekNumber = Math.ceil(diffMs / weekMs);
...
function getWorkSchedule({ start, end }, countWorkDays, countOffDays) {
  ...
  const startDate = new Date(startYear, Number(startMonth) - 1, startDay);

  const [endDay, endMonth, endYear] = end.split('-');
  const endDate = new Date(endYear, Number(endMonth) - 1, endDay);
